...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['updateEmployeeAccountStatus'])){
    $passwordConfirmation = mysqli_real_escape_string($connection,test_input($_POST['adminPasswordConfirmation']));
    $isUpdateSuccessfull = false;

    // Check if admin password is input successfully
    if (password_verify($passwordConfirmation, $_SESSION['loggedAccount']['Password'])) {
        if (updateEmployeeAccountStatus($_POST['selectedAccountID'],$_POST['status'])){
...
<div class="container d-flex">
    ...
    <div class="content d-flex justify-content-center">
        <div class="main-content p-5">
            ...
            <div class="row px-5">
                <?php 
                if (isset($_POST['updateEmployeeAccountStatus'])){
                    if($isUpdateSuccessfull){
                        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
                                Employee account update status success.
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>';
                    } else {
                        echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                                Update employee account status unsuccessful due to wrong password. Please try again.
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>';
                    }
                }
